...
public class CommonApi implements ModpackApi {
    ...
    public SearchResult searchMod(SearchFilters searchFilters, SearchResult previousPageResult) {
        ...
        for(int i = 0; i < mModpackApis.length; i++) {
            if (i == 0 && !searchFilters.includeModrinth) continue;
    ...
    private @NonNull ModpackApi getModpackApi(int apiSource) {
        ...
        throw new UnsupportedOperationException("Unsupported API source: " + apiSource);
    }
    ...
    private @NonNull ModpackApi getModpackApi(Activity activity, Uri zipUri){
        ...
        try {
            ...
            while ((zipEntry = zipInputStream.getNextEntry()) != null) {
                if(zipEntry.getName().equals(modrinthPackInfoFileName)) {
                    return mModrinthApi;
                }
            }
        } catch (Exception e) {
        ...
        throw new IllegalArgumentException("Zip provided does not contain a Modrinth manifest file");
    }
