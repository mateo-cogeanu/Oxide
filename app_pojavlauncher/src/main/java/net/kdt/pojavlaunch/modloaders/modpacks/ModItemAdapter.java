...
public class ModItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements TaskCountListener {
    ...
    private final LinkedHashMap<String, QueuedInstall> mQueuedInstalls = new LinkedHashMap<>();
    ...
    public int getQueuedSelectionCount() {
        return mQueuedInstalls.size();
    }

    public void installQueued(android.content.Context context) {
        if (mQueuedInstalls.isEmpty()) return;
        ProgressLayout.setProgress(ProgressLayout.INSTALL_MODPACK, 0, R.string.global_waiting);
        LinkedHashMap<String, QueuedInstall> queuedCopy = new LinkedHashMap<>(mQueuedInstalls);
        mQueuedInstalls.clear();
        notifyDataSetChanged();
        mSearchResultCallback.onSelectionChanged(0);
        PojavApplication.sExecutorService.execute(() -> {
            try {
                for (Map.Entry<String, QueuedInstall> entry : queuedCopy.entrySet()) {
                    QueuedInstall queuedInstall = entry.getValue();
                    mModpackApi.installMod(queuedInstall.modDetail, queuedInstall.selectedVersion);
                }
                Tools.runOnUiThread(() -> Toast.makeText(
                        context,
                        R.string.mod_install_batch_success,
                        Toast.LENGTH_LONG
                ).show());
            } catch (Exception e) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            view.setOnClickListener(v -> {
                if(!hasExtended()){
                    ...
                    mExtendedButton.setOnClickListener(v1 -> toggleQueuedSelection());
        ...
        private void setStateDetailed(ModDetail detailedItem) {
            if(detailedItem != null) {
                ...
                int compatibleIndex = ModCompatibility.findCompatibleVersionIndex(
                        detailedItem.mcVersionNames,
                        detailedItem.versionLoaders,
                        detailedItem.versionNames,
                        mSearchFilters == null ? null : mSearchFilters.mcVersion,
                        mSearchFilters == null ? null : mSearchFilters.modLoader
                );
                if (compatibleIndex >= 0 && compatibleIndex < detailedItem.versionNames.length) {
                    mExtendedSpinner.setSelection(compatibleIndex, false);
                }
                updateQueuedButtonLabel();
            } else {
                closeDetailedView();
        ...
        private void openDetailedView() {
            ...
            mExtendedLayout.setTranslationY(16f);
            mExtendedLayout.animate().alpha(1f).translationY(0f).setDuration(180).start();
        }

        private void closeDetailedView(){
            mExtendedLayout.animate().cancel();
            mExtendedLayout.setAlpha(1f);
            mExtendedLayout.setTranslationY(0f);
        ...
        private void toggleQueuedSelection() {
            if (mModDetail == null) return;
            int selectedVersion = Math.max(mExtendedSpinner.getSelectedItemPosition(), 0);
            String queueKey = mModDetail.id + "#" + selectedVersion;
            if (mQueuedInstalls.containsKey(queueKey)) {
                mQueuedInstalls.remove(queueKey);
            } else {
                mQueuedInstalls.put(queueKey, new QueuedInstall(mModDetail, selectedVersion));
            }
            updateQueuedButtonLabel();
            mSearchResultCallback.onSelectionChanged(mQueuedInstalls.size());
        }

        private void updateQueuedButtonLabel() {
            if (mExtendedButton == null || mModDetail == null) return;
            int selectedVersion = Math.max(mExtendedSpinner.getSelectedItemPosition(), 0);
            String queueKey = mModDetail.id + "#" + selectedVersion;
            mExtendedButton.setText(mQueuedInstalls.containsKey(queueKey)
                    ? R.string.mod_download_queued
                    : R.string.mod_add_to_queue);
        }
    }
    ...
    private static class QueuedInstall {
        final ModDetail modDetail;
        final int selectedVersion;

        private QueuedInstall(ModDetail modDetail, int selectedVersion) {
